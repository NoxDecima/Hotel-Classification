...
if [ "$HOSTNAME" != "cn99" ] && [ "$HOSTNAME" != "cn47" ] && [ "$HOSTNAME" != "cn48" ]; then
  ...
  VENV_DIR=/scratch/$USER/virtual_environments/hotel-classification
fi
...
python -m pip install -r "$PROJECT_DIR"/requirements.txt -f https://download.pytorch.org/whl/cu111/torch_stable.html
